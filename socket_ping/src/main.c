...
void displayRecvPacket(char *recv_packet, uint32_t received_data,
                       TcpHdr *tcphdr, SceNetIcmpHeader *recv_icmphdr,
                       char *recv_payload);
void displaySentPacket(char *packet, uint32_t packet_size,
                       SceNetIcmpHeader *icmphdr, char *payload);
...
int main(int argc, char *argv[]) {
  ...
  sfd = sceNetSocket("ping_test", SCE_NET_AF_INET, SCE_NET_SOCK_RAW,
  ...
  retval = sceNetSetsockopt(sfd, SCE_NET_SOL_SOCKET, SCE_NET_SO_BROADCAST,
                            (const void *)&on, sizeof(on));
  if (retval == -1)
    goto exit;
  printf("Allow socket to broadcast.\n");

  icmp.icmp_struct.hdr.type =
      SCE_NET_ICMP_TYPE_ECHO_REQUEST; /* set icmp type to echo request */
  ...
  strncpy(icmp.icmp_struct.payload, "Random Payload in ping",
          ICMP_MIN_PAYLOAD); /* NB: strncpy fills the remainder of the buffer
                                with zeroes */
  icmp.icmp_struct.hdr.checksum =
      in_cksum(icmp.icmp_u16buff, sizeof(IcmpPacket)); /* compute checksum */

  serv_addr.sin_family = SCE_NET_AF_INET; /* set packet to IPv4 */
  serv_addr.sin_addr = dst_addr;          /* set destination address */
  memset(&serv_addr.sin_zero, 0,
         sizeof(serv_addr.sin_zero)); /* fill sin_zero with zeroes */

  /* Send data */
  sent_data =
      sceNetSendto(sfd, icmp.icmp_packet, sizeof(IcmpPacket), 0,
                   (SceNetSockaddr *)&serv_addr, sizeof(SceNetSockaddr));
  if (sent_data < 1)
    goto exit; /* send failed */

  printf("Data sent:\n");
  printf("----------\n\n");
  displaySentPacket(icmp.icmp_packet, sizeof(IcmpPacket), &icmp.icmp_struct.hdr,
                    icmp.icmp_struct.payload); /* Display colored data */
  ...
  printf("\n\nReceiving data.\n");
  received_data = sceNetRecvfrom(sfd, tcp.tcp_packet, sizeof(TcpPacket),
                                 SCE_NET_MSG_WAITALL, &from_addr,
  ...
  printf("--------------\n\n");
  displayRecvPacket(tcp.tcp_packet, sizeof(TcpPacket), &tcp.tcp_struct.hdr,
                    &tcp.tcp_struct.icmp.hdr,
                    tcp.tcp_struct.icmp.payload); /* display colored data */
...
void displayRecvPacket(char *recv_packet, uint32_t received_data,
  ...
  printf("ICMP checksum = 0x%04X, is %s\n\n", recv_icmphdr->checksum,
         (in_cksum((uint16_t *)(recv_packet + 20), received_data - 20))
             ? "invalid"
             : "valid");

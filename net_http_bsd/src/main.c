...
int main(__attribute__((unused)) int argc,
  ...
  connect(fd,
          (const struct sockaddr *)&((struct sockaddr_in){
              .sin_family = AF_INET,
              .sin_port = htons(80),
              .sin_addr.s_addr = *(long *)(gethostbyname(url[0])->h_addr)}),
  ...
  while (read(fd, &c, sizeof(c)) > 0 && pos < sizeof(line)) {
    ...
    if (c == '\n') {      // end of line reached
      psvDebugScreenPrintf("%.*s\n", pos,
                           line); // printf the received line into the screen
  ...
  for (unsigned i = 0; i < sizeof(base);
       i += 4) // for every RGBA bytes pointed by the screen "base" adress
    write(fdump, base + i,
          3); // write the RGB part (3 bytes) into "screen.data"
  close(fdump);
#endif

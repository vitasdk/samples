...
#define FROM_3BIT(c, dark)                                                     \
  (((!!((c)&4)) << 23) | ((!!((c)&2)) << 15) | ((!!((c)&1)) << 7) |            \
   (dark ? 0 : 0x7F7F7F))
#define FROM_6BIT(c)                                                           \
  ((((c) % 6) * (51 << 16)) | ((((c) / 6) % 6) * (51 << 8)) |                  \
   ((((c) / 36) % 6) * 51))
...
static size_t psvDebugScreenEscape(const unsigned char *str) {
  for (unsigned i = 0, argc = 0, arg[32] = {0};
       argc < (sizeof(arg) / sizeof(*arg)) && str[i] != '\0'; i++)
    switch (str[i]) {
    case '0':
    case '1':
    case '2':
    ...
    case 'K':
      if (arg[0] == 0)
        CLEARSCRN(coordY, coordY + F.size_h, coordX,
                  SCREEN_WIDTH); // from curosr to end
      if (arg[0] == 1)
        CLEARSCRN(coordY, coordY + F.size_h, 0,
                  coordX); // from begining to cursor
      if (arg[0] == 2)
      ...
      for (unsigned c = 0; c <= argc; c++) {
        ...
        if (unit == 9) { // reset FG or BG
          *color = mode & 1 ? defaultFg : defaultBg;
                   ...
                   (arg[c + 1] ==
                    5)) { // 8bit : [0-15][16-231][232-256] color map
          c += 2;
...
int psvDebugScreenInit() {
  ...
  SceUID displayblock = sceKernelAllocMemBlock(
      "display", SCE_KERNEL_MEMBLOCK_TYPE_USER_CDRAM_RW, SCREEN_FB_SIZE, NULL);
  sceKernelGetMemBlockBase(displayblock, (void **)&base);
  SceDisplayFrameBuf frame = {sizeof(frame), base,         SCREEN_FB_WIDTH, 0,
...
int psvDebugScreenPuts(const char *_text) {
  ...
  for (c = 0; text[c]; c++) {
    ...
    for (int row = 0, mask = 1 << 7; row < F.height;
         row++, vram += SCREEN_FB_WIDTH) {
      ...
      for (uint32_t *pixel = vram + F.width, col = F.width; col < F.size_w;
           col++)
...
void psvDebugScreenSetFgColor(uint32_t rgb) {
  psvDebugScreenPrintf("\e[38;2;%lu;%lu;%lum", (rgb >> 16) & 0xFF,
                       (rgb >> 8) & 0xFF, rgb & 0xFF);
}
void psvDebugScreenSetBgColor(uint32_t rgb) {
  psvDebugScreenPrintf("\e[48;2;%lu;%lu;%lum", (rgb >> 16) & 0xFF,
                       (rgb >> 8) & 0xFF, rgb & 0xFF);
}
#undef F
#endif

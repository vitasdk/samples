...
int main(int argc, char *argv[]) {
  ...
  } arg = {psvDebugScreenMutex, &psvDebugScreenFrameBuf};
  ...
  for (;;) {
    ...
    if (ONPRESS(SCE_CTRL_UP)) {
      modid = sceKernelLoadStartModule(suprx_path, sizeof(arg), &arg, 0, NULL,
                                       &status);
    }
    if (ONPRESS(SCE_CTRL_DOWN)) {
      ret = sceKernelStopUnloadModule(modid, sizeof(arg_fini), arg_fini, 0,
                                      NULL, &status);
      modid = 0;
    }
    if (ONPRESS(SCE_CTRL_LEFT)) {
      ...
      ret = sceKernelGetModuleList(0x80 | 0x1, modids,
                                   &num); // 0/1 = user 0x80=system
      ...
      for (int n = 0; n < num; n++) {
        ...
        printf("%08lX <%X-%lX> [%s%s%s%s] %-15s %-40s %4i %4i %8p\n", mi.modid,
               sizeof(mi), mi.size, mi.unk28 ? "i" : " ",
               mi.module_start ? "s" : " ", mi.module_stop ? "s" : " ",
               mi.module_exit ? "e" : " ", mi.module_name, mi.path,
               mi.extabBtm - mi.extabTop, mi.exidxBtm - mi.exidxTop,
               mi.tlsInit);

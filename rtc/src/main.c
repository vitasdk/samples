...
int main(int argc, char *argv[]) {
  ...
  printf("Your Local Time:%04d/%02d/%02d %02d:%02d:%02d Valid:%i\n",
         sceRtcGetYear(&time), sceRtcGetMonth(&time), sceRtcGetDay(&time),
         sceRtcGetHour(&time), sceRtcGetMinute(&time), sceRtcGetSecond(&time),
         sceRtcGetMicrosecond(&time), sceRtcCheckValid(&time));

  printf("We are the %ith days of the week (Sun:0)\n",
         sceRtcGetDayOfWeek(sceRtcGetYear(&time), sceRtcGetMonth(&time),
                            sceRtcGetDay(&time)));
  ...
  printf("     Current %s:%llu\n", sceRtcGetCurrentTick(&current) ? "FAIL" : "",
         current.tick);
  ...
  SceRtcTick add1000, us1000, sec120, min120, hour48, days32, week64, month13,
      year1000;
  printf("      T+1000 %s:%llu\n",
         sceRtcTickAddTicks(&add1000, &current, 1000) ? "FAIL" : "",
         add1000.tick);
  printf("     Us+1000 %s:%llu\n",
         sceRtcTickAddMicroseconds(&us1000, &current, 1000) ? "FAIL" : "",
         us1000.tick);
  printf("      S+120  %s:%llu\n",
         sceRtcTickAddSeconds(&sec120, &current, 120) ? "FAIL" : "",
         sec120.tick);
  printf("      M+120  %s:%llu\n",
         sceRtcTickAddMinutes(&min120, &current, 120) ? "FAIL" : "",
         min120.tick);
  printf("      H+48   %s:%llu\n",
  ...
  printf("      M+13   %s:%llu\n",
         sceRtcTickAddMonths(&month13, &current, 13) ? "FAIL" : "",
         month13.tick);
  printf("      Y+1000 %s:%llu\n",
         sceRtcTickAddYears(&year1000, &current, 1000) ? "FAIL" : "",
         year1000.tick);

  char rfc3339_loc[128], rfc3339_12[128], rfc2822_loc[128], rfc2822_12[128];
  printf("RFC3339local %s:%s\n",
         sceRtcFormatRFC3339LocalTime(rfc3339_loc, &utc) ? "FAIL" : "",
         rfc3339_loc);
  printf("RFC3339 + 2  %s:%s\n",
         sceRtcFormatRFC3339(rfc3339_12, &utc, -120) ? "FAIL" : "", rfc3339_12);
  printf("RFC2822local %s:%s\n",
         sceRtcFormatRFC2822LocalTime(rfc2822_loc, &utc) ? "FAIL" : "",
         rfc2822_loc);
  ...
  printf("RFC3339_1    %s:%-26s->%llu\n",
         sceRtcParseRFC3339(&parse_ex1, rfc_ex1) ? "FAIL" : "", rfc_ex1,
         parse_ex1.tick);
  printf("RFC3339_2    %s:%-26s->%llu\n",
         sceRtcParseRFC3339(&parse_ex2, rfc_ex2) ? "FAIL" : "", rfc_ex2,
         parse_ex2.tick);
  printf("RFC3339_3    %s:%-26s->%llu\n",
         sceRtcParseRFC3339(&parse_ex3, rfc_ex3) ? "FAIL" : "", rfc_ex3,
         parse_ex3.tick);
  ...
  printf("datetime_1   %s:%-26s->%llu\n",
         sceRtcParseDateTime(&parsedDate_ex1, datetime_ex1) ? "FAIL" : "",
         datetime_ex1, parsedDate_ex1);
  printf("datetime_2   %s:%-26s->%llu\n",
         sceRtcParseDateTime(&parsedDate_ex2, datetime_ex2) ? "FAIL" : "",
         datetime_ex2, parsedDate_ex2);
  printf("datetime_3   %s:%-26s->%llu\n",
         sceRtcParseDateTime(&parsedDate_ex3, datetime_ex3) ? "FAIL" : "",
         datetime_ex3, parsedDate_ex3);

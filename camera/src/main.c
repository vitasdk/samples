...
int main(void) {
  ...
  SceUID memblock = sceKernelAllocMemBlock(
      "camera", SCE_KERNEL_MEMBLOCK_TYPE_USER_CDRAM_RW, 256 * 1024 * 5, NULL);
  sceKernelGetMemBlockBase(memblock, &base);

  SceDisplayFrameBuf dbuf = {sizeof(SceDisplayFrameBuf),
                             base,
                             DISPLAY_WIDTH,
                             0,
                             DISPLAY_WIDTH,
  ...
  do {
    int cur_sat = 0, cur_rev = 0, cur_efx = 0, cur_iso = 0, cur_zom = 10,
        cur_nim = 0, cur_wbl = 0;
    size_t res_table[][2] = {{0, 0},     {640, 480}, {320, 240},
                             {160, 120}, {352, 288}, {176, 144},
                             {480, 272}, {640, 360}, {640, 360}};
    ...
    do {
      ...
      sceCameraSetGain(cur_cam, (ctrl_press.ly < 100)
                                    ? ctrl_press.ly / 6
                                    : 0); // 0..16 //don't seem to work
      ...
      if (ctrl_press.buttons & SCE_CTRL_CROSS)
        sceCameraSetISO(
            cur_cam, (int[]){1, 100, 200, 400}[cur_iso = ((cur_iso + 1) % 4)]);
      ...
      */
      if (sceCameraIsActive(cur_cam) > 0) {
        ...
        !(ctrl_press.buttons & (SCE_CTRL_UP | SCE_CTRL_DOWN | SCE_CTRL_LEFT |
                                SCE_CTRL_RIGHT | SCE_CTRL_START)));
